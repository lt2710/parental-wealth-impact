...
work <- NULL
salary_data <- cbind(work_2014$id%>% as.character(),
                     work_2014$hhid%>% as.character(),
                     pay = "missing")%>%as.data.frame()
for (i in 1:nrow(work_2014)) {
  ...
  for (j in 0:18) {
    ...
    if (is.na(work_2014[[start]][i]) == FALSE &
      ...
      if (job %in% c("1 Own Agricultural Production and Business") &
          is_empty(work_raw_2014[[paste("f101a_1_",j,"_", sep = "")]][i])=="FALSE"){
        salary <- work_raw_2014[[paste("f101a_1_",j,"_", sep = "")]][i]%>%as.numeric()*12
      } else if (job %in% c("2 Agricultural Employment",
                            "3 Non-agricultural Employment")&
          is_empty(work_raw_2014[[paste("f112_1_",j,"_", sep = "")]][i])=="FALSE"){
        salary <- work_raw_2014[[paste("f112_1_",j,"_", sep = "")]][i]%>%as.numeric()*12
      } else if (job %in% c("4 Non-agricultural self-employment")&
          is_empty(work_raw_2014[[paste("f116_1_",j,"_", sep = "")]][i])=="FALSE"){
        salary <- work_raw_2014[[paste("f116_1_",j,"_", sep = "")]][i]%>%as.numeric()*12
      } else if (job %in% c("5 Unpaid Household Business Help")){
        salary <- 0
      } else if (job %in% c("6 Army")&
          is_empty(work_raw_2014[[paste("f117a_",j,"_", sep = "")]][i])=="FALSE"){
        salary <- work_raw_2014[[paste("f117a_",j,"_", sep = "")]][i]%>%as.numeric()*12
      }
